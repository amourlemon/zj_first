 ...
 function deepClone(originalVal, map = new WeakMap()) {
   // 判读如果是 set 类型
   ...
   if (map.has(originalVal)) {
     return map.get(originalVal);
   }
   // 实现数组的拷贝
   const newObj = Array.isArray(originalVal) ? [] : {};
   map.set(originalVal, newObj);
   for (const key in originalVal) {
     newObj[key] = deepClone(originalVal[key], map);
   }
   ...
   for (const skey of skeys) {
     ...
     newObj[skey] = deepClone(originalVal[skey], map);
   }
   return newObj
 }
